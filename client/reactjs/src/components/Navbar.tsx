        ...
        <>
            ...
            >
                ...
                <div className="hidden md:flex items-center gap-8 transition duration-500">
                    ...
                    {
                        isLoggedIn ? <Link to='/my-generation' className="hover:text-pink-400 transition">My Generations</Link>
                        : <Link to='#' className="hover:text-pink-400 transition">About</Link>
                    }
            ...
            <div className={`fixed inset-0 z-100 bg-black/40 backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-400 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
                    <Link onClick={() => setIsOpen(false)} to='/' className="hover:text-pink-400 transition">Home</Link>
                    
                    <Link onClick={() => setIsOpen(false)} to='/generate' className="hover:text-pink-400 transition">Generate</Link>

                    {
                    isLoggedIn ? 
                    <Link onClick={() => setIsOpen(false)} to='/my-generation' className="hover:text-pink-400 transition">My Generations</Link> 
                    : <Link onClick={() => setIsOpen(false)} to='#' className="hover:text-pink-400 transition">About</Link>
                    }
                    
                    <Link onClick={() => setIsOpen(false)} to='#' className="hover:text-pink-400 transition">Contact us</Link>

                    {isLoggedIn ? 
                    <button onClick={() => {setIsOpen(false); logout()}}>
                        Logout
                    </button>
                    : <Link onClick={() => setIsOpen(false)} to='/login' className="hover:text-pink-400 transition">Login</Link>
                    }
