...
const cloudflareSizes: Record<string, [number, number]> = {
    '16:9': [1024, 576],
    '1:1': [1024, 1024],
    '9:16': [576, 1024],
}
...
// square: the image is 1:1 regardless of the requested aspect ratio (fallback model)
type GeneratedImage = { data: string, mimeType: string, square?: boolean };

const detectMimeType = (base64: string) =>
    base64.startsWith('/9j/') ? 'image/jpeg' : base64.startsWith('UklGR') ? 'image/webp' : 'image/png';

const runCloudflareModel = async (model: string, body: FormData | string): Promise<string> => {
    ...
    if (!response.ok || !image) {
        const error: any = new Error(`Image generation failed: ${json?.errors?.[0]?.message || `HTTP ${response.status}`}`);
...
const generateWithCloudflare = async (prompt: string, aspectRatio: string): Promise<GeneratedImage> => {
    ...
    try {
        ...
        if (error.code !== CLOUDFLARE_FLAGGED_OUTPUT) throw error;
...
const generateWithGemini = async (prompt: string, aspectRatio: string): Promise<GeneratedImage> => {
